...
def criar_pasta_do_credor(credor_nome):
    nome_limpo = re.sub(r'[<>:"/\\|?*]', '', credor_nome).strip().replace(' ', '_').upper()

    raiz_projeto = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
    pasta_base = os.path.join(raiz_projeto, 'relatorios', nome_limpo)

    # Verificar se a pasta já existe. Se não, cria a pasta
    if not os.path.exists(pasta_base):
        os.makedirs(pasta_base, exist_ok=True)
    
    return pasta_base

def save_to_excel(dados_empenhos, credor_nome="CREDOR", ano_inicio="XXXX", ano_fim="XXXX"):
    if not dados_empenhos:
        print("Nenhum dado de empenho para salvar.")
        return None

    try:
        pasta_destino = criar_pasta_do_credor(credor_nome)
        nome_arquivo = f"{credor_nome}_{ano_inicio}-{ano_fim}_DIARIAS.xlsx"
...
def save_to_pdf(dados_empenhos, valor_total, credor_nome, ano_inicio, ano_fim, cidade, orgao):
    pasta_destino = criar_pasta_do_credor(credor_nome)
    nome_arquivo = f"{credor_nome}_{ano_inicio}-{ano_fim}_DIARIAS.pdf"
    ...
    elements.append(Paragraph(f"Diárias de Viagens - {credor_nome}", styles["Title"]))
